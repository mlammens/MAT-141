...
covid = read.csv("~/Dropbox/00_Sort/deaths_by_race_ethnicity__all_age_groups.csv")
```
...
```{r}
covid$obs_exp = ((covid$Death_Percentage - covid$Pop_Percentage)^2) / covid$Pop_Percentage
```
...
pchisq(q = chi_sq, df = 6, lower.tail = FALSE)
```
