...
It will be easier to do the subsequent analyses if we filter out the height values that are `NA`s. 
We'll do this by using what we know about accessing vectors and data frames by indexs.

```{r}
# Find all the NA values in the heights column
# The `is.na` function returns a logical vector, with TRUE for values that are NA and FALSE otherwise
heights_na = is.na(yrbss$height)

# To get the values that are numbers, negate the heights_na vector
heights_num = !heights_na

# Now use the `which` function to determine the values that are TRUE
heights_num_true = which(heights_num)

# Finally, use the vector we just created as row indices to make a new table
yrbss_noNAs = yrbss[heights_num_true, ]
```
...
```{r}
set.seed(1981)
yrbss_samp <- yrbss_noNAs[sample(x = 1:nrow(yrbss_noNAs), replace = FALSE, size = 100), ]
```
...
for(x in 1:100){
  ...
  mean_samp <- mean(yrbss_samp$height)
...
Let's calculate the Confidence Interval for our estimate of mean height, using our very first sample of the yrbss_noNAs data.

First, get the sample again.

```{r}
set.seed(1981)
yrbss_samp <- yrbss_noNAs[sample(x = 1:nrow(yrbss_noNAs), replace = FALSE, size = 10), ]
```

Calculate our sample means.

```{r}
mean_yrbss_samp <- mean(yrbss_samp$height)
```
...
```{r}
se_yrbss_samp <- sd(yrbss_samp$height) / sqrt(nrow(yrbss_samp))
```
