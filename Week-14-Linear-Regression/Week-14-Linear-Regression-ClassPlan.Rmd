...
Here we will work with a data set of weight measurements of whole cats (kg) and their hearts (g). 
We will use the whole-cat weight to predict heart weight using linear regression.
...
# Filter cats so we are only using the female cats
...
```{r}
ggplot(data = cats_female, aes(x = Bwt, y = Hwt)) +
  geom_point() +
  xlab("Body weight (kg)") +
  ylab("Heart weight (g)")
```
...
```{r}
cats_female_lm = lm(data = cats_female, Hwt ~ Bwt)

summary(cats_female_lm)
```
